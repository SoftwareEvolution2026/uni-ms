...
class PasswordChangeIntegrationTest {
    ...
    void wrongCurrentPasswordIsRejected() throws Exception {
        String token = token("staff@uni.ms", "Staff123!");
        mockMvc.perform(put("/api/v1/auth/password")
    ...
    void userChangesPasswordThenLogsInWithNewOne() throws Exception {
        String token = token("lecturer@uni.ms", "Lecturer123!");

        mockMvc.perform(put("/api/v1/auth/password")
                        .header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"currentPassword":"Lecturer123!","newPassword":"Lecturer456!"}"""))
        ...
        mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"email":"lecturer@uni.ms","password":"Lecturer123!"}"""))
        ...
        mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"email":"lecturer@uni.ms","password":"Lecturer456!"}"""))
                .andExpect(status().isOk());
    }
}
